...
exact `0.1.0a37` prerelease:

```bash
pip install outlabs-taskq==0.1.0a37

# with extras:
pip install "outlabs-taskq[http]==0.1.0a37"
pip install "outlabs-taskq[outlabs]==0.1.0a37"
```
...
There is no `install()` helper. `db migrate` applies packaged SQL (`0001`–`0043`)
under an advisory lock; `verify` is the read-only drift oracle (catalog,
grants, checksums) for SQL contract **0.6.7**. Run both with an administrative
