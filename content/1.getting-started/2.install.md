...
commands target the `0.1.0a40` candidate and become valid only after its
immutable release artifacts exist. Until then, install the reviewed local wheel
by path; do not let a package resolver substitute another prerelease.

```bash
pip install outlabs-taskq==0.1.0a40

# with extras:
pip install "outlabs-taskq[http]==0.1.0a40"
pip install "outlabs-taskq[outlabs]==0.1.0a40"
```
