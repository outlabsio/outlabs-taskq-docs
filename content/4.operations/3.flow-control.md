...
The flow-control plane introduced in SQL contracts 0.4.0–0.6.7 and retained in
0.6.12 regulates *how fast and how much* work a queue takes on, and stops the fleet from hammering a failing downstream. Every feature is **off by default and configured per queue** — a queue that sets nothing gets the same control behavior as before: nothing gates, throttles, ages, or breaks its work. When a gate declines work it returns a typed `throttled` claim verdict carrying `retry_after_seconds`, so workers sleep exactly as told rather than guessing.
...
Contract `0.6.7` introduced enqueue depth admission from those exact counters in
constant time. Earlier contracts could scan into a deep queue once for every
job admitted, making a large planning transaction progressively slower. That
behavior remains in `0.6.12`. Keep queue counters healthy and follow the
[runtime-first upgrade sequence](/operations/runtime-first-upgrades) before
applying any later contract.
