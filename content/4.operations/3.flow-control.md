...
The flow-control plane (SQL contracts 0.4.0–0.6.7) regulates *how fast and how much* work a queue takes on, and stops the fleet from hammering a failing downstream. Every feature is **off by default and configured per queue** — a queue that sets nothing gets the same control behavior as before: nothing gates, throttles, ages, or breaks its work. When a gate declines work it returns a typed `throttled` claim verdict carrying `retry_after_seconds`, so workers sleep exactly as told rather than guessing.
...
Contract `0.6.7` also makes enqueue depth admission read those exact counters in
constant time. Earlier contracts could scan into a deep queue once for every
job admitted, making a large planning transaction progressively slower. Keep
queue counters healthy and upgrade every producer, worker, scheduler, and
operator runtime to a version that accepts `0.6.7` before applying migration
`0043`.
