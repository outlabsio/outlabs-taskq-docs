...
taskq queue set-breaker mail --off  # disable
```
```sql
...
::warning
The breaker is **streak-based** — a single success resets the streak. A downstream that fails *intermittently* (fail, succeed, fail, succeed) never trips. It is also blind to slow-but-succeeding downstreams. Use it for hard-down failure modes; rate/window and latency tripping are on the roadmap.
::

Start with a **conservative** threshold (5–10) — too low false-trips a heterogeneous queue. Set `cooldown-seconds` to the downstream's realistic recovery time.
...
There is no dashboard or event stream for the breaker yet — read state directly:

```sql
-- Breaker + rate + aging + ramp state for one queue:
...
-- Any breaker currently open, system-wide:
SELECT queue, breaker_tripped_at FROM taskq.queue_flow WHERE breaker_state <> 'closed';

-- Live levels, throughput, drain-rate/ETA:
SELECT * FROM taskq.queue_counters WHERE queue = 'mail';
SELECT * FROM taskq.queue_health('mail');   -- note: does not include breaker state
```
