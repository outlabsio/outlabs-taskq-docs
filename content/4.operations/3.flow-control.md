...
The flow-control plane (SQL contracts 0.4.0–0.6.4) regulates *how fast and how much* work a queue takes on, and stops the fleet from hammering a failing downstream. Every feature is **off by default and configured per queue** — a queue that sets nothing behaves exactly as it did before. When a gate declines work it returns a typed `throttled` claim verdict carrying `retry_after_seconds`, so workers sleep exactly as told rather than guessing.
